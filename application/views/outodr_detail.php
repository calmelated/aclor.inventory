...
?>
    ...
    <div class="control-group">
        <label class="control-label">Reqisition Date</label>
        <div class="controls">
    ...
    <div class="control-group">
        <label class="control-label">Finished Product #</label>
        <div class="controls">
    ...
    <div class="form-actions">
        ...
        <?php } else if($act == "detail_edit") { ?>
            ...
            <button type="reset" class="btn btn-link">Cancel</button>
        <?php } else { ?>
            <a href="outodr/add" class="btn btn-primary">
        ...
        <?php if(($this->session->userdata['user_auth'] > 1) &&  /* auth:2 admin */
            ...
            <a href="outodr/id_del/<?=$outodr['id']; ?>" class="btn btn-danger pull-right">
        ...
        <?php if(($this->session->userdata['user_auth'] > 1) &&  /* auth:2 admin */
                 ($act == "detail")) { ?>
            <a href="outodr/id_edit/<?=$outodr['id']; ?>" class="btn btn-info pull-right">
...
<script>
    $(document).ready(function() {
        ...
        <?php } else if ($act == "detail_add") { ?>
            ...
            <?php for($i = 0; $i < MAX_ITEMS; $i = $i + 1) { ?>
                $("input[name='item_num_<?=$i;?>']").attr({'autocomplete': 'off', 'data-source': items});
                $("input[name='unit_<?=$i;?>']").attr({'autocomplete': 'off', 'data-source': units});
                $("input[name='unit1_<?=$i;?>']").attr({'autocomplete': 'off', 'data-source': units});
            <?php } ?>
